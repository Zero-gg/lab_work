...
bool isValidSubject(const char* str) {
    ...
    // Проверка на наличие цифры в конце (семестр/часть)
    // Разрешено: "Предмет X", где X - цифра от 1 до 9.
    if (isdigit((unsigned char)str[len - 1])) {
        // Если последний символ - цифра
        if (str[len - 1] == '0') {
             printf("! Ошибка: Номер семестра/части может быть только от 1 до 9.\n");
             return false;
        }
        
        // Она должна быть отделена пробелом
        if (str[len - 2] != ' ') {
             printf("! Ошибка: Цифра в конце должна быть отделена пробелом (например, 'Математика 1').\n");
             printf("! Примечание: Разрешена только одна цифра от 1 до 9.\n");
             return false;
        }
        
        // Проверяем часть названия до цифры
        check_len = len - 2;
        if (check_len < 2) {
             printf("! Ошибка: Название предмета перед цифрой слишком короткое.\n");
             return false;
        }
    }
    ...
    for (size_t i = 0; i < check_len; i++) {
        ...
        if (!isCyrillic(c) && c != ' ') {
             printf("! Ошибка: Недопустимый символ '%c'. В названии предмета разрешены только русские буквы и пробелы (цифра допускается только одна и строго в конце).\n", c);
             return false;
        }
    }
    return true;
    ...
    do {
        ...
        while (true) {
            ...
            if (!isValidSurname(buf)) {
                // Сообщение об ошибке уже выведено в функции isValidSurname
                continue;
            }
        ...
        while (true) {
            ...
            if (!isValidPost(buf)) {
                // Сообщение об ошибке уже выведено в функции isValidPost
                continue;
            }
        ...
        while (true) {
            ...
            if (!isValidSubject(buf)) {
                // Сообщение об ошибке уже выведено в функции isValidSubject
                continue;
            }
            // Ввод корректен
            normalizeSentenceCase(buf);
    ...
    do {
        ...
        while (true) {
            ...
            if (!isValidGroupNumber(buf)) {
                // Сообщение об ошибке уже выведено в функции isValidGroupNumber
                continue;
            }
