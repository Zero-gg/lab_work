...
void Main_Menu2() {
    ...
    while (true) {
        if (menu_flag) {
            ...
            // printf("1. Посмотреть, есть ли файлы для формирования сведений.\n");
...
#include <set> // Не забудь добавить это в начало файла, если нет
...
void Display_Subject_Data() {
    ...
    std::map<std::string, std::vector<long>> exam_index;
    //printf("\n[ЗАГРУЗКА] Анализ доступных предметов...\n");
    Build_Exam_Index(exam_index);
...
bool Check_File_Availability() {
    ...
    // 3. (ВАЖНО!) Закрываем файлы, если они открылись
    if (teachers_exist) {
        fclose(teachers_file);
    }
    if (exams_exist) {
